...
const steps = [
  { num: "01", label: "Tell Us", desc: "Share your systems challenges" },
  { num: "02", label: "Schedule", desc: "Pick a time that works" },
  { num: "03", label: "Scope", desc: "Discuss your build together" },
];
        ...
        <div className="section-container">
          ...
          >
            ...
            <div className="flex flex-wrap gap-4">
              {trustLogos.map(name => (
                <div key={name} className="px-4 py-2 border border-border rounded-lg bg-card/50 text-[13px] font-heading font-medium text-text-muted">
                  {name}
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1.5fr] gap-12 md:gap-20 items-start">
            <RevealOnScroll>
              <div className="md:sticky md:top-24">
                <h3 className="font-heading text-xl font-semibold tracking-[-0.02em] mb-7">
                  What the Consultation Includes
                </h3>
                <div className="flex flex-col gap-5">
                  {consultationIncludes.map((item, i) => (
                    <div key={i} className="flex gap-3.5 items-start">
                      <div className="w-7 h-7 bg-accent-light border border-primary/20 rounded-lg flex items-center justify-center flex-shrink-0 text-primary mt-0.5">
                        <Icons.Check />
                      </div>
                      <div>
              ...
              <AnimatePresence mode="wait">
                {submitted ? (
                  <motion.div
                    ...
                    className="text-center py-16 px-10 border border-border rounded-2xl relative overflow-hidden"
                  >
                    {/* Radial burst */}
                    <motion.div
                      ...
                      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full bg-primary/20"
                    />
                    <div className="relative z-10">
                      <motion.div
                        ...
                        className="w-14 h-14 bg-accent-light rounded-full flex items-center justify-center mx-auto mb-6 text-primary"
                      >
                      ...
                      <p className="text-[15px] text-text-secondary mb-7">
                        Thank you, {formData.name}. A member of our team will contact you within one business day to schedule your consultation.
                      </p>
                      <Link
                        to="/"
                        className="font-heading text-[13px] font-medium bg-transparent text-text-primary border border-border-strong rounded-lg px-[18px] py-[9px] inline-flex items-center gap-1.5 hover:bg-surface transition-all"
                      >
                        Return to Home
                      </Link>
                    </div>
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-5">
                      <div className="space-y-1.5">
                        ...
                        <select
                          className="w-full h-11 border border-border rounded-lg px-3.5 font-body text-sm text-text-primary bg-card outline-none focus:border-primary focus:ring-[3px] focus:ring-accent-light transition-all appearance-none"
                    ...
                    <div className="space-y-1.5 mt-5">
                      ...
                      <textarea
                        className="w-full min-h-[120px] border border-border rounded-lg p-3.5 font-body text-sm text-text-primary bg-card outline-none resize-y focus:border-primary focus:ring-[3px] focus:ring-accent-light transition-all"
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.99 }}
                      className="w-full mt-6 font-heading text-[15px] font-medium bg-primary text-primary-foreground rounded-lg py-3.5 flex items-center justify-center gap-2 hover:bg-primary/90 hover:shadow-[0_8px_24px_rgba(79,70,229,0.25)] transition-all disabled:opacity-60"
                    >
        ...
        <div className="section-container max-w-[720px]">
          <RevealOnScroll>
            <div className="text-center mb-12">
              ...
              <h2 className="font-heading text-[clamp(24px,3vw,36px)] font-semibold tracking-[-0.03em] leading-[1.1] text-text-primary">
            ...
            <Accordion type="single" collapsible className="space-y-3">
              {faqs.map((faq, i) => (
                <AccordionItem key={i} value={`faq-${i}`} className="border border-border rounded-xl px-6 bg-card">
                  <AccordionTrigger className="text-[15px] font-heading font-medium text-text-primary hover:no-underline py-5">
                    {faq.q}
                  </AccordionTrigger>
                  <AccordionContent className="text-[14px] text-text-secondary leading-[1.7] pb-5">
                    {faq.a}
                  </AccordionContent>
                </AccordionItem>
              ))}
