...
import {
  ...
  ChevronRight, Calendar, Tag, FileText, Star, FolderOpen
} from "lucide-react";
...
export default function ResourceList() {
  ...
  const toggle = async (id: string, published: boolean) => {
    ...
    if (item) await logActivity(published ? "unpublished_resource" : "published_resource", "resource", item.title, id);
    fetchData();
  };
  ...
  const remove = async (id: string) => {
    ...
    if (item) await logActivity("deleted_resource", "resource", item.title, id);
    fetchData();
  };
  ...
  const bulkAction = async (action: "publish" | "unpublish" | "delete") => {
    ...
    if (action === "delete" && !confirm(`Delete ${ids.length} resources?`)) return;
    if (action === "publish") await supabase.from("resources").update({ published: true }).in("id", ids);
    else if (action === "unpublish") await supabase.from("resources").update({ published: false }).in("id", ids);
    else await supabase.from("resources").delete().in("id", ids);
    await logActivity(`bulk_${action}_resources`, "resource", `${ids.length} resources`);
    setSelected(new Set());
    fetchData();
  };
  ...
  return (
    <div className="p-4 sm:p-8 space-y-8 max-w-[1400px]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-heading font-bold tracking-tight text-foreground">Resources</h1>
          <p className="text-muted-foreground text-sm">Manage downloadable assets, whitepapers, and guides.</p>
        </div>
      ...
      {selected.size > 0 && (
        <motion.div
          ...
          className="flex items-center justify-between bg-emerald-600 text-white px-6 py-3 rounded-2xl shadow-lg"
        >
          <div className="flex items-center gap-4 text-sm font-medium">
            ...
            />
            <span>{selected.size} resources selected</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="py-24 text-center">
          ...
          <h3 className="text-lg font-heading font-bold">No resources found</h3>
          <p className="text-muted-foreground text-sm">Try adjusting your filters or create a new resource.</p>
        </div>
      ) : viewMode === 'list' ? (
                    ...
                    <tr key={item.id} className="group hover:bg-muted/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="min-w-[240px]">
                          <div className="flex items-center gap-2">
                            <Link href={`/admin/resources/${item.id}`} className="text-sm font-bold text-foreground hover:text-primary transition-colors line-clamp-1">
                              {item.title}
                            </Link>
          ...
          {filtered.map((item) => (
            <Card key={item.id} className="border-none shadow-sm rounded-3xl overflow-hidden bg-card group">
              <CardContent className="p-6">
                ...
                <Link href={`/admin/resources/${item.id}`} className="block group-hover:text-primary transition-colors">
                ...
                <div className="flex items-center gap-3 mt-6 pt-4 border-t border-border">
                  <div className="flex-1 text-[11px] text-muted-foreground">
                    ...
                    {item.featured && <Star size={10} className="inline ml-1 text-amber-500 fill-amber-500" />}
                  </div>
