export const Icons = {
  ArrowRight: ({ size = 14 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 14 14" fill="none">
      <path d="M1 7h12M7 1l6 6-6 6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  ArrowLeft: ({ size = 14 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 14 14" fill="none">
      <path d="M13 7H1M7 13l-6-6 6-6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  ExternalLink: ({ size = 12 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 12 12" fill="none">
      <path d="M5 2H2a1 1 0 00-1 1v7a1 1 0 001 1h7a1 1 0 001-1V7M7 1h4v4M5 7L11 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  Menu: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M3 5h14M3 10h14M3 15h14" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  X: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M4 4l12 12M16 4L4 16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  Integration: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <rect x="2" y="2" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.5"/>
      <rect x="12" y="2" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.5"/>
      <rect x="2" y="12" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.5"/>
      <rect x="12" y="12" width="6" height="6" rx="1.5" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M8 5h4M5 8v4M15 8v4M8 15h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  Database: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <ellipse cx="10" cy="5" rx="7" ry="2.5" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M3 5v10c0 1.38 3.13 2.5 7 2.5s7-1.12 7-2.5V5" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M3 10c0 1.38 3.13 2.5 7 2.5s7-1.12 7-2.5" stroke="currentColor" strokeWidth="1.5"/>
    </svg>
  ),
  Workflow: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <circle cx="4" cy="5" r="2" stroke="currentColor" strokeWidth="1.5"/>
      <circle cx="16" cy="5" r="2" stroke="currentColor" strokeWidth="1.5"/>
      <circle cx="4" cy="15" r="2" stroke="currentColor" strokeWidth="1.5"/>
      <circle cx="16" cy="15" r="2" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M6 5h4M10 5h2M6 15h8M4 7v6M16 7v6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  Intelligence: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M10 2a6 6 0 016 6c0 2.5-1.5 4.7-3.7 5.7L12 16H8l-.3-2.3C5.5 12.7 4 10.5 4 8a6 6 0 016-6z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/>
      <path d="M8 16v2h4v-2" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/>
    </svg>
  ),
  Monitor: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <rect x="2" y="3" width="16" height="11" rx="2" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M7 17h6M10 14v3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
      <path d="M6 9l2 2 3-4 2 2" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  Shield: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M10 2L3 5v6c0 4.4 3.1 8.1 7 9 3.9-.9 7-4.6 7-9V5L10 2z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/>
      <path d="M7 10l2 2 4-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  Building: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <rect x="3" y="3" width="14" height="15" rx="1.5" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M7 7h2M11 7h2M7 11h2M11 11h2M9 18v-4h2v4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  Globe: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <circle cx="10" cy="10" r="8" stroke="currentColor" strokeWidth="1.5"/>
      <path d="M2 10h16M10 2c-2.5 2.5-4 5.1-4 8s1.5 5.5 4 8c2.5-2.5 4-5.1 4-8s-1.5-5.5-4-8z" stroke="currentColor" strokeWidth="1.5"/>
    </svg>
  ),
  Check: ({ size = 16 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none">
      <path d="M3 8l3.5 3.5L13 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  ),
  FileText: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M5 3h7l4 4v11a1 1 0 01-1 1H5a1 1 0 01-1-1V4a1 1 0 011-1z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/>
      <path d="M12 3v5h5M7 9h6M7 12h6M7 15h4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  BarChart: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M4 14V9M8 14V6M12 14v-4M16 14V4" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
      <path d="M2 17h16" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
    </svg>
  ),
  Zap: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 20 20" fill="none">
      <path d="M11 2L4 11h6l-1 7 7-9h-6l1-7z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round"/>
    </svg>
  ),
  BookOpen: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/>
      <path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
    </svg>
  ),
  Home: ({ size = 20 }: { size?: number } = {}) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
      <path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/>
      <polyline points="9 22 9 12 15 12 15 22"/>
    </svg>
  ),
};

export type IconName = keyof typeof Icons;
